...
public class ProvidersDAOImpl implements ProvidersDAO {
	...
	public List<ProvidersEntity> getAll() {
		return sessionFactory.getCurrentSession().createQuery("from ProvidersEntity where providerStatus='" + 1 + "'")
				.list();
	}
	...
	public boolean remove(String providersId) {
		try {
			sessionFactory.getCurrentSession().createQuery(
					"update ProvidersEntity set providerStatus='" + 0 + "' where providersId='" + providersId + "'")
					.executeUpdate();
		} catch (Exception e) {
			return false;
		}
